...
if __name__=="__main__":
    ...
    for D in range(1, 10**3 + 1):
        if D**0.5 %1 == 0:
            continue

        A, period_length = continued_fraction(D)
        curr_next = 0

        i = 1
        while True:

            frac = final_fraction(A[:i])

            if equation(frac.numerator, frac.denominator, D) == 1:
            ...
            if i > len(A):
                A.append(A[curr_next + 1])
                curr_next = (curr_next + 1)%period_length
